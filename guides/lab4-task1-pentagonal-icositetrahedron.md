...
### Почему нужен особый порядок отрисовки

Если просто включить blending, полупрозрачные грани будут смешиваться некорректно. Проблема в глубине:

- Включённый depth test **блокирует** отрисовку фрагмента, если перед ним уже что-то есть в z-буфере
- Ближняя полупрозрачная грань запишет своё значение глубины в z-буфер и заблокирует дальнюю грань — та не отрисуется вовсе, хотя должна просвечивать

Решение — рисовать полупрозрачные объекты с **отключённой записью** в z-буфер (`glDepthMask(false)`), но сохранить чтение из него. Тогда обе грани нарисуются, но важен их **порядок**: дальняя должна быть нарисована раньше ближней, иначе смешивание будет визуально неверным.

Пентагональный икоситетраэдр — **выпуклый** объект, и это ключевое свойство: при любом положении камеры нелицевые грани (повёрнутые от камеры) всегда находятся дальше лицевых (повёрнутых к камере). Правильный порядок гарантирован — достаточно нарисовать объект **дважды**:

1. **Первый проход:** `glCullFace(GL_FRONT)` — отбракуй лицевые (ближние), нарисуй только дальние
2. **Второй проход:** `glCullFace(GL_BACK)` — отбракуй нелицевые (дальние), нарисуй только ближние

## 6.1. GLRenderer — настраиваем в `onSurfaceCreated`

В `PentagonalIcositetrahedronGLRenderer`, в методе `onSurfaceCreated`, добавь после `glEnable(GL_DEPTH_TEST)`:
...
`glEnable(GL_CULL_FACE)` — включаем отбраковку граней. Без этого `glCullFace` ничего не сделает.
...
## 6.3. GLRenderer — двухпроходная отрисовка в `onDrawFrame`

Замени вызов `_drawable.draw(...)` в `onDrawFrame` на двухпроходную схему:

```
// Отключаем запись в z-буфер — дальние грани не должны блокировать ближние
glDepthMask(false)

// Проход 1: рисуем только дальние (нелицевые) грани
glCullFace(GL_FRONT)
_drawable.draw(...)

// Проход 2: рисуем только ближние (лицевые) грани поверх
glCullFace(GL_BACK)
_drawable.draw(...)

// Восстанавливаем запись в z-буфер
glDepthMask(true)
```

`glCullFace` и `glDepthMask` управляют глобальным состоянием OpenGL — их вызывают в `GLRenderer`, а не внутри `Renderer.draw`. Сам `draw` вызывается дважды подряд без изменений.

## 6.4. Проверка

Запусти — фигура полупрозрачная и без артефактов. Повращай: дальние грани корректно просвечивают сквозь ближние при любом ракурсе.
...
| | Направленный (шаг 5) | Точечный (шаг 7) |
|---|---|---|
